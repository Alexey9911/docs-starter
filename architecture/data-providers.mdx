...
CryptoGuard integrates with external data sources for token and address risk checks. This page lists the supported chains for these checks.
...
Most signals refresh within minutes. For time-sensitive threats, CryptoGuard prioritizes real-time feeds and applies short cache windows so you see changes quickly.

## Provider validation

Signals are never trusted in isolation. CryptoGuard cross-checks multiple sources, down-weights stale feeds, and applies internal heuristics before a decision is surfaced.
